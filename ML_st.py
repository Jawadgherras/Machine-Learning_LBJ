...
from ScrapingDonnees_st import ESPNWebScraper, Conversion_de_types, Donnees
...
def knn_avec_ou_sans_ACP(df, ACP=False, n_neighbors=[3, 5, 7, 9], weights=["uniform", "distance"], p=[1, 2], ACP_composantes=[2, 3, 4, 5], test_size=0.2, random_state=42):
    ...
    meilleurs_parametres=print(f"Meilleur hyperparamètres : {grid_search.best_params_}")
    meilleur_score=grid_search.best_score_
    
    y_pred=grid_search.predict(X_test)
    test_mse=mean_squared_error(y_test, y_pred)
    
    mse_test=print(f"Erreur quadratique moyenne, données de test : {test_mse}")
    
    
    console = Console()
    table = Table(title="Résultats du modèle Knn_acp", show_lines=True)
    
    table.add_column("Modèle", justify="center", style="cyan", no_wrap=True)
    table.add_column("Meilleurs Hyperparamètres", justify="center", style="magenta")
    table.add_column("Score (CV)", justify="center", style="green")
    table.add_column("MSE Test", justify="center", style="red")
    ...
    #return meilleurs_parametres, meilleur_score, mse_test
...
def regressions_lineaires(df, type_de_modele="lineaire", alpha=[0.001, 0.01, 0.1, 1, 10], test_size=0.2, random_state=42):
    ...
    console = Console()
    table = Table(title="Résultats de regression lineaire", show_lines=True)
...
def regression_polynomiales(df, type_de_modele="lineaire", alpha=[0.001, 0.01, 0.1, 1, 10], degres=[1, 2, 3], test_size=0.2, random_state=42):
    ...
    console = Console()
    table = Table(title="Résultats de regression polynomiale 'lineaire' ", show_lines=True)
    ...
    table.add_row(
        "Résultats de regression polynomiale 'lineaire'",
        str(grid_search.best_params_),  
        f"{meilleur_score}",  
        f"{test_mse}"         
    )
...
def support_vector_regression(df, C=[0.1, 1, 10, 100], epsilon=[0.1, 0.01, 0.001], kernel=["linear", "poly", "rbf"], test_size=0.2, random_state=42):
    ...
    console = Console()
    table = Table(title="Résultats du modèle SVR", show_lines=True)
...
def gradient_boosting(df, nbr_estimateurs=[200, 300, 1000], taux_apprentissage=[0.1, 0.01, 0.001], profondeur_max=[3, 5, 7, 9], test_size=0.2, random_state=42):
    ...
    meilleurs_parametres=print(f"Meilleur hyperparamètres : {grid_grb.best_params_}")
    ...
    console = Console()
    table = Table(title="Résultats du Gradient Boosting", show_lines=True)
...
def random_forest(df,n_estimators=[200, 300, 400], max_depth=[3, 5, 7, 9], min_samples_split=[2, 3, 4], test_size=0.2, random_state=42):
    ...
    meilleurs_parametres=print(f"Meilleur hyperparamètres : {grid_rfr.best_params_}")
    ...
    console = Console()
    table = Table(title="Résultats du Random Forest", show_lines=True)
...
def reseaux_neurones(df, hidden_layer_sizes=[(50,), (100,), (50,50)], fct_activation=["relu","tanh", "logistic"], solver=["lbfgs", "adam"], test_size=0.2, random_state=42):
    ...
    meilleurs_parametres=print(f"Meilleur hyperparamètres : {grid_mlp.best_params_}")
    ...
    console = Console()
    table = Table(title="Résultats du Reseau de neurones", show_lines=True)
...
def comparaison_resultats(df, modele_teste=None, test_size=0.2, random_state=42, parametres_knn=None, parametres_knn_acp=None, parametres_lineaire=None, parametres_poly_lr=None, parametres_poly_lasso=None, parametres_poly_ridge=None, parametres_svr=None, parametres_gbr=None, parametres_rfr=None, parametres_mlp=None):
    ...
    for nom_du_modele, modele, param_grid in modeles_etudies:
        ...
        resultats.append([nom_du_modele, meilleurs_parametres, test_mse])

        # Ajouter les résultats dans la table rich
        table.add_row(nom_du_modele, str(meilleurs_parametres), str(test_mse))
